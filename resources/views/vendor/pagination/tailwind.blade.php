        ...
        @foreach ($elements as $element)
            @if (is_array($element))
                @foreach ($element as $page => $url)
                    <button type="button" class="min-h-[38px] min-w-[38px] flex justify-center items-center text-gray-800 hover:bg-gray-100 py-2 px-3 text-sm rounded-lg focus:outline-none focus:bg-gray-100 {{ $paginator->currentPage() == $page ? 'bg-gray-200 cursor-default' : '' }}" {{ $paginator->currentPage() == $page ? 'disabled' : '' }}>
                        {{ $page }}
                    </button>
                @endforeach
            @endif
